...
class RCNN(BaseModel):
    ...
    def layer_encoder(self):
        ...
        with tf.variable_scope('cnn_encoder'):
            ...
            for kernel_size in self.config.kernel_sizes:
                with tf.variable_scope('conv-%d' % kernel_size) as scope:
                    output = tf.layers.conv1d(self.s_emb, self.config.filter_size, 
                                              kernel_size, padding='SAME', name='conv1d-%d' % kernel_size)
        ...
        with tf.variable_scope('rnn_encoder'):
            ...
            output, _ = rnn(self.output, seq_len=self.s_len)
