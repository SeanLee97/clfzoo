...
def model(cfg=None, training=False):
    ...
    if not isinstance(cfg, ConfigTransformer):
        raise ValueError('The config of Transformer should inherited from ConfigTransformer')

    config = cfg
    instance = Instance(config, training=training)

    cfg.logger.info("Init model...")
    clf_model = Transformer(instance.vocab, cfg)
